...
Packages declare a pass mark (`adlcp:masteryscore`, `minNormalizedMeasure`, cmi5 `masteryScore`) only under `completion.mode: "quiz"` with a quiz verdict, so an LMS cannot reach its own verdict ahead of the course. cmi5 `moveOn` is `CompletedAndPassed` whenever the course sends a verdict at all, so a failed learner is denied credit in every delivery mode.
