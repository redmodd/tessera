...
A verdict no longer stands in for completion: SCORM 1.2 holds `passed` back until the course completes. Packages declare a pass mark only under `completion.mode: "quiz"` with a quiz verdict, and cmi5 `moveOn` is `CompletedAndPassed` whenever the course can send a verdict.
