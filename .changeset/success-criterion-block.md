...
A verdict no longer stands in for completion: `passed` waits for the course to complete, in every standard. Packages declare a pass mark only under `completion.mode: "quiz"` with a quiz verdict, and cmi5 `moveOn` is `CompletedAndPassed` whenever the course is sure to send a verdict.
