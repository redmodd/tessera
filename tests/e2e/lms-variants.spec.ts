...
/**
 * `free` (passingScore 70) under an LMS configured with mastery_score 60, so a
 * 67 passes only if the course takes the LMS threshold.
 */
test.describe.serial('SCORM 1.2 LMS mastery_score', () => {
  const PORT = 5314;
  const BASE = `http://localhost:${PORT}`;
  let preview: ChildProcess;
  ...
  test.beforeEach(async ({ page }) =>
    installScorm12Mock(page, { 'cmi.student_data.mastery_score': '60' }),
  );

  test('overrides scoring.passingScore for lesson_status', async ({ page }) => {
    await openQuiz(page, BASE, 'Graded Assessment');
    ...
    expect(Number(data['cmi.core.score.raw'])).toBeLessThan(70);
    expect(await page.evaluate(() => (window as any).__scormErrors)).toEqual(
      [],
    );
  });
});
