...
`pageScore` is what a page shows about itself: a graded quiz page's score, or the weighted mean of the standalone questions answered on a page without a quiz. A page of standalone questions renders no score on its own, so read it and print one. Three things to know:
...
- **Only graded work counts.** Practice answers and an ungraded practice quiz are ignored, so a practice page reads `undefined`, never a score. The page reads exactly what it contributes to `gradedScore`.
